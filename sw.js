...
self.addEventListener("activate", (e) => {
  e.waitUntil(
    caches.keys()
      .then((keys) => Promise.all(keys.filter((k) => k !== CACHE && k !== "mv-osm").map((k) => caches.delete(k))))
...
self.addEventListener("fetch", (e) => {
  ...
  // Tuiles/style/sprite OpenStreetMap (OpenFreeMap) : cache-first → utilisables hors-ligne une fois téléchargés.
  if (url.hostname === "tiles.openfreemap.org") {
    e.respondWith(
      caches.open("mv-osm").then((c) =>
        c.match(req).then((hit) =>
          hit ||
          fetch(req).then((r) => {
            if (r && r.status === 200) { const copy = r.clone(); c.put(req, copy).catch(() => {}); }
            return r;
          }).catch(() => hit) // hors-ligne sans cache → MapLibre gère l'absence
        )
      )
    );
    return;
